...
WEB_SEARCH — Route to: web search agent (Direct Lightpanda, zero RAG)
Triggers (any search keywords):
  "news", "today", "latest", "breaking", "current events",
  "what happened", "this week", "right now", "live", "stock price",
  "weather", "sports score", "recently", "trending", "headlines",
  "search the web", "google", "web search", "internet search"
Action: web(query="{original_message}")

────────────────────────────────────────────────────────

FILESYSTEM — Route to: executor directly (LOW risk)
Triggers (Paths or file access):
  "ls", "dir", "cat", "head", "tail", "pwd", "nvidia-smi", "df -h",
  r"C:\"", r"/mnt/", r"/home/", "access folder", "list contents"
Action: executor(command="{original_message}", risk_override="low")
...
Action: planner(goal="{original_message}")
...
Action: executor(command="{original_message}")
...
If NO intent matches OR any agent returns failure:
  Action: respond_direct(message="{answer from parametric knowledge}")
