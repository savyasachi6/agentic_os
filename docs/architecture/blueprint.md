...
Residing primarily within `core`, these modules represent specialized business logic:
...
* **Persistence First**: Every thought, command result, and state change is logged to the `Execution Tree` in `memory`.
