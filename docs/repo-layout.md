...
| :--- | :--- |
| `agent_core` | Shared base classes, types, and the main state-machine graph (`agents/core/coordinator.py`). |
| `agents` | Specialist agent worker implementations (Executor, RAG, Code, etc.). |
...
| `rag` | `CognitiveRetriever` and vector search components (`agent_core/rag/`). |
| `rl_router` | Standalone RL-based contextual bandit routing service. |
...
| `sandbox` | Isolated code execution environments (`browser_tools.py`). |
