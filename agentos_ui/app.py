...
# --- Monorepo Shim ---
# This shim ensures that the Streamlit application can discover and import
# other modules within the Agentic OS monorepo structure.
_HERE = os.path.dirname(os.path.abspath(__file__))
_ROOT = os.path.dirname(_HERE)
for subpkg in ["agentos_core", "agentos_memory", "agentos_skills"]:
...
def get_router_stats():
    """Fetches bandit diagnostics from the gateway's router stats proxy."""
    try:
        response = requests.get(f"{CORE_API_URL}/router/stats", timeout=5)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
        st.error(f"Failed to fetch router stats: {e}")
    return None
    ...
    if page == "💬 Terminal":
        ...
        if st.button("🔄 Sync Skills", use_container_width=True):
            with st.spinner("Indexing..."):
                import subprocess
                # Run the indexer from agentos_core
                result = subprocess.run([sys.executable, os.path.join(_ROOT, "agentos_core", "main.py"), "index"], capture_output=True, text=True)
...
if page == "💬 Terminal":
    ...
    if stats and "arm_stats" in stats:
        ...
        st.info("RL Router not detected or no stats available. Start the `agentic_rl_router` service.")
