...
from rl_router.domain.drift import CUSUMDriftDetector, DriftResult
...
class LinUCBBandit:
    ...
    def select_arm(self, context: np.ndarray, epsilon: float = 0.01) -> Tuple[int, List[float], bool]:
        """Select arm with highest safety-aware UCB score.

        Returns (best_arm, ucb_scores, is_exploration).
        """
        # --- Epsilon-greedy floor (Safety) ---
        if epsilon > 0 and np.random.random() < epsilon:
            with self._lock:
                random_arm = int(np.random.randint(self.n_arms))
                # Still compute UCBs for telemetry
                ...
                for a in range(self.n_arms):
                    ...
                    mean = float(theta @ x)
                    # Variance guard: max(0, var) + 1e-10 for numerical stability
                    variance = float(x @ A_inv @ x)
                    exploration = self.alpha * float(np.sqrt(max(0.0, variance) + 1e-10))
                    viol_rate = self._violation_rate(a)
                    ucb_scores.append(mean - self.viol_lambda * viol_rate + exploration)
                return random_arm, ucb_scores, True
        ...
        with self._lock:
            ...
            best_variance = float(x @ A_inv @ x)
            best_expl = self.alpha * float(np.sqrt(max(0.0, best_variance) + 1e-10))
    ...
    def update(
        ...
        if drift_result.drift_detected:
            # Soft reset (matrix decay) to allow re-learning after drift.
            # 30% retention (retain_fraction=0.7 implies decay to 0.7*orig)
            # Justification: Retaining 30% of the curvature (A) and mean (b) 
            # provides a "warm prior" that weights the model's structure but 
            # increases the variance (A_inv) enough to trigger re-exploration.
            self.soft_reset(arm, retain_fraction=0.7)
    ...
    def get_condition_number(self, arm: int) -> float:
        """Matrix conditioning diagnostic."""
        with self._lock:
            return float(np.linalg.cond(self._A_inv[arm]))
    ...
    def get_arm_stats(self, arm: int) -> dict:
        ...
        return {
            ...
            "condition_number": round(self.get_condition_number(arm), 2),
            "performance": "O(d^2) Sherman-Morrison",  # Manual verification of complexity
        }
