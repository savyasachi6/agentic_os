...
from rl_router.domain.models import LinguisticFeatures
...
class ContextFeatureBuilder:
    ...
    def build(
        ...
        *,
        ...
        metadata_scale: float = 10.0,
    ) -> np.ndarray:
        """Build context vector from raw components."""

        # 1. Embedding (Normalized)
        ...
        # Normalize to unit sphere to stabilize variance
        norm = np.linalg.norm(emb)
        if norm > 0:
            emb = emb / norm

        # 2. Intent logits (Scaled)
        ...
        intents = intents * metadata_scale

        # 3. Linguistic features (Scaled)
        ling = extract_linguistic_features(query_text)
        ling_vec = np.array(ling.to_vector(), dtype=np.float64) * metadata_scale

        # 4. Session stats (Scaled)
        corpus_hash = (
