...
The `skills` project defines how an agent learns and retrieves specialized capabilities. It manages the lifecycle of "Skills"—structured Markdown documents (`SKILL.md`) that contain instructions, examples, and logic—and provides a high-performance retrieval engine for context injection.
...
- Python 3.11+
- **memory** (must be set up and running for vector storage)
...
Ensure your `.env` file (managed by `core` or shared) specifies the `SKILLS_DIR`.
