...
@pytest.fixture
def mock_ollama():
    with patch("memory.vector_store.ollama") as mock:
        mock.embeddings.return_value = {"embedding": [0.1, 0.2, 0.3]}
        yield mock

@pytest.fixture
def mock_db_conn():
    with patch("memory.vector_store.get_db_connection") as mock_get_conn:
        mock_conn = MagicMock()
        mock_cur = MagicMock()
...
class TestVectorStore:
    def test_generate_embedding(self, mock_ollama):
        ...
        self.assertEqual(len(emb), model_settings.embed_dim)
