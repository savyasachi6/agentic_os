...
from tools.tools import build_tool_registry
...
def call_model(state: AgentState) -> dict:
    ...
    try:
        ...
        # Defensive check for Pydantic V2 validation: content must be a string or list
        if response.content is None:
            response.content = json.dumps({"response": "I encountered an empty response from the model. Please rephrase."})
        elif not isinstance(response.content, (str, list)):
