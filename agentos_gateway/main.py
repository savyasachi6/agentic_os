...
Usage:
    python -m agentos_gateway.main cli    — interactive REPL
    python -m agentos_gateway.main serve  — start FastAPI server
    python -m agentos_gateway.main index  — re-index all skills into pgvector
"""
...
for subpkg in ["agentos_core", "agentos_memory", "agentos_skills", "agentos_router"]:
    _p = os.path.join(_ROOT, subpkg)
    if os.path.exists(_p) and _p not in sys.path:
        sys.path.insert(0, _p)
# Also add root so 'agentos_gateway' is discoverable if run as a script
