...
def _parse_code_action(response_text: str) -> Optional[Tuple[str, str]]:
    """
    Extended balanced-paren parser that also handles `|`-delimited payloads
    for write_file and propose_diff without breaking on pipe characters.
    Falls back to parse_react_action for simple cases.
    """
...
class CodeAgentWorker:
    ...
    def __init__(self, model_name: Optional[str] = None, workspace_root: Optional[str] = None):
        ...
        # Root path for relative file resolution and safety checks
    ...
    def _load_prompt(self):
        ...
        prompt_path = os.path.join(root_dir, "core", "prompts", "code_agent_prompt.md")
    ...
    async def _process_task(self, task: Node):
        ...
        try:
            max_iterations = 6
            pending_diff: Optional[Tuple[str, str]] = None  # (path, diff_text)
            ...
            for _ in range(max_iterations):
                ...
                if not action_data:
                    self.tree_store.update_node_status(
                ...
                if action_type in ("complete", "done", "respond", "finish"):
                    ...
                    self.tree_store.update_node_status(task.id, NodeStatus.DONE, result={"summary": payload})
                    print(f"[CodeAgent] Finished task {task.id}")
                    return

                # --- read_file ---
                elif action_type == "read_file":
                    obs = self._read_file(payload)
                    print(f"[CodeAgent] read_file({payload[:80]}) → {len(obs)} chars")

                # --- list_dir ---
                elif action_type == "list_dir":
                    obs = self._list_dir(payload)
                    print(f"[CodeAgent] list_dir({payload[:80]})")

                # --- propose_diff ---
                elif action_type == "propose_diff":
                    parts = payload.split("|", 1)
                    if len(parts) == 2:
                        pending_diff = (parts[0].strip(), parts[1].strip())
                        obs = f"[Diff proposed for {pending_diff[0]}]\n{pending_diff[1]}"
                        print(f"[CodeAgent] Diff proposed for {pending_diff[0]}")
                    else:
                        obs = "[Error: propose_diff requires path|diff format]"
                ...
                # --- write_file ---
                    ...
                    if len(parts) == 2:
                        ...
                        obs = self._write_file(path, content)
                        pending_diff = None
                    else:
                ...
                # --- run_command ---
                    ...
                    obs = self._run_command(payload)
                    print(f"[CodeAgent] run_command: {payload[:80]}")
    ...
    async def run_forever(self, poll_interval: float = 2.0):
        """Infinite loop polling queue for 'tools' / code tasks."""
        self._running = True
        print("[CodeAgent] Worker started.")
        while self._running:
            try:
                task = self.tree_store.dequeue_task(agent_role=AgentRole.TOOLS)
                if task:
                    await self._process_task(task)
                else:
                    await asyncio.sleep(poll_interval)
            except Exception as e:
                print(f"[CodeAgent] Polling error: {e}")
                await asyncio.sleep(poll_interval)
