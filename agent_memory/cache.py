...
def _make_redis_client():
    ...
    try:
        from agent_config import redis_settings
        ...
        client = redis_lib.Redis(
            ...
            socket_connect_timeout=1,        # fail fast on startup
            socket_timeout=1,
            retry_on_timeout=True,
        )
        client.ping()                        # validate connection
        logger.info("[cache] Redis L0 connected at %s:%s", redis_settings.host, redis_settings.port)
        return client
...
class FractalCache:
    ...
    def _l0_set(self, query_hash: str, payload: Dict[str, Any]):
        ...
        try:
            from agent_config import redis_settings
            ttl = redis_settings.ttl_seconds
