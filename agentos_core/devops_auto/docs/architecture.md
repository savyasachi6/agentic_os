...
- **Isolation**: All tests and deployments must run in sandboxed containers (coordinated with `agentos_core/sandbox`).
- **Auditability**: Every git commit and deployment action is logged to the central `agentos_memory` for audit history.
