# AgentOS .NET Tools Node

The .NET Tools Node is a secure, per-call JWT-authenticated sidecar API designed for the AgentOS AI Agent. It executes high-risk operations like shell commands and file system modifications on behalf of the agent, ensuring strict, scoped authorization at the boundary.
...
The API requires a JWT bearer token with audience `agentos-tools` and the appropriate scope. For local development, the Python Agent's `ToolClient` automatically mints and caches symmetric tokens relying on `appsettings.Development.json`.
